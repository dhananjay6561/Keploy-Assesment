...
If you’ve ever built a backend API and found  
**“mocking your database, setting up interfaces, and manually defining expected outputs is tedious”**,  
this tutorial is for you.

In this guide, we’ll take a simple **Go + Echo + PostgreSQL** URL shortener app and use **Keploy** to automatically generate and run API test cases **without writing test code or mocks**.
...
<Callout
  title="Don’t have Keploy installed yet?"
...
We’ll use an official Keploy Go sample.
...
We’ll keep it simple by running Postgres in a Docker container.
...
In a new terminal, we’ll use Keploy to record our API interactions.
...
Keploy records real **API calls**, so let’s make some.
...
## Step 6: Verify the recorded test cases:
...
Your app thinks it’s talking to PostgreSQL.

It’s not. Keploy is handling that.
